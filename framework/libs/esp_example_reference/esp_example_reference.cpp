...
void Reference::taskEntry(void *arg)
{
    Reference *self = static_cast<Reference *>(arg);
...
esp_err_t Reference::ensureMutex(void)
{
    if (mutex_ != nullptr) {
        return ESP_OK;
    }
...
esp_err_t Reference::setName(const char *name)
{
...
void Reference::resetState(void)
{
    counter_ = 0;
}

void Reference::step(void)
{
    counter_++;

    reference_callback_t callback = callback_;
    if (callback != nullptr) {
        callback(*this);
    }
}

esp_err_t Reference::init(const char *name, uint32_t interval_ms)
{
    if (interval_ms == 0U) {
        return ESP_ERR_INVALID_ARG;
    }
...
esp_err_t Reference::start(uint32_t interval_ms, UBaseType_t priority, BaseType_t core)
{
    if (interval_ms == 0U) {
        return ESP_ERR_INVALID_ARG;
    }
...
bool Reference::ready(void) const
{
    if (mutex_ == nullptr || !lock()) {
        return configured_;
    }
...
bool Reference::started(void) const
{
    if (mutex_ == nullptr || !lock()) {
        return started_;
    }
...
esp_err_t Reference::intervalMs(uint32_t interval_ms)
{
    if (interval_ms == 0U) {
        return ESP_ERR_INVALID_ARG;
    }
...
esp_err_t Reference::onTick(reference_callback_t callback)
{
    if (mutex_ != nullptr && !lock()) {
        return ESP_ERR_INVALID_STATE;
    }
...
const char *Reference::name(void) const
{
    return name_;
}

uint32_t Reference::intervalMs(void) const
{
    if (mutex_ == nullptr || !lock()) {
        return interval_ms_;
    }
...
uint32_t Reference::counter(void) const
{
    if (mutex_ == nullptr || !lock()) {
        return counter_;
    }
