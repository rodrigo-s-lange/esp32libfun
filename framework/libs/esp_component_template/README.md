...
1. Copy `framework/libs/esp_component_template` into a separate `esp_*` library repository.
2. Rename these items first:
   ...
   - `template_callback_t`
   - global object `templ`
3. Before changing transport logic, read one external library that uses the same kind of core dependency.
4. For digital interrupts, PWM, or pulse counting, inspect the matching core module first:
...
5. If the library owns mutable state or exposes an optional task, protect that state explicitly
...
6. If the library uses a shared core resource, rely on the core wrapper and keep ownership boundaries clear
...
7. Replace the example step/callback logic with your real device or service logic
8. Keep the lifecycle contract only if the library really benefits from it
9. Add optional `*_at.cpp` integration only if the library exposes AT commands
