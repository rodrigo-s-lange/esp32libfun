...
esp_err_t W5500::validateIpString(const char *value)
{
...
esp_err_t W5500::applyHostname(void)
{
...
esp_err_t W5500::applyNetifConfig(void)
{
    if (s_eth_netif == nullptr) {
...
esp_err_t W5500::ip(const char *ip) const
{
    esp_err_t err = ensureSyncPrimitives();
    if (err != ESP_OK) {
        return err;
...
esp_err_t W5500::gateway(const char *gateway) const
{
    esp_err_t err = ensureSyncPrimitives();
    if (err != ESP_OK) {
        return err;
...
esp_err_t W5500::subnet(const char *subnet) const
{
    esp_err_t err = ensureSyncPrimitives();
    if (err != ESP_OK) {
        return err;
